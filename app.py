...
# Page config
st.set_page_config(
    page_title="VISA STATUS PREDICTOR AND PROCESSING TIME ESTIMATOR 🚀",
    page_icon="✈️",
    layout="wide",
...
with col2:
    st.markdown('<h1 class="main-header">✈️ Visa Status prediction and Time Processing Estimator </h1>', unsafe_allow_html=True)
...
with tab2:
    ...
    if uploaded_file:
        ...
        if st.button("🔮 Predict All", key="bulk"):
            ...
            df['expected_date'] = pd.to_datetime("today") + pd.to_timedelta(df['predicted_days'], unit='D')
            df['expected_date'] = df['expected_date'].dt.date
